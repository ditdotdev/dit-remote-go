...
Dit remote provider for connecting d3 CLI to dit-remote-server. This provider enables d3 to use dit-remote-server as a remote storage backend, providing a "GitHub for d3" experience with web UI, organizations, authentication, and collaboration features.

## Architecture

This remote provider allows d3 CLI users to:
- Push commits to dit-remote-server via HTTP/HTTPS
- Pull and clone repositories from dit-remote-server
...
- `dit-remote-go` → dit-remote-server (GitHub-style, with web UI and collaboration)

## Quick Start

### Installation

The dit remote is included with d3 CLI v1.0.0+. No separate installation needed.

### Usage

```bash
# Add dit remote
d3 remote add origin https://data.dit.io/myorg/myrepo

# Push commits
d3 commit -m "Initial commit"
d3 push origin master

# Clone from dit-remote-server
d3 clone https://data.dit.io/myorg/myrepo
...
- **HTTP/HTTPS protocol** - Works with dit-remote-server APIs
...
https://data.dit.io/myorg/myrepo   (production)
```
